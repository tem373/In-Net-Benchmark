...
cat edited_$1.txt | sed -e 's/Depth = //' -e 's/expt_type = loss //' -e 's/ mean_delay_or_loss = /, /' -e 's/ dist_type =/,/' -e 's/ loss_probability = /, /' -e 's/ loss_type = /, /' -e 's/ num_probes = /, /' -e 's/ num_trials = /, /' -e 's/avg. tomography error =  /, /' -e 's/ %//' -e 's/avg. in-network error =  /, /' -e 's/ trials//' | sed -e 'N;N;s/\n/ /g' | sed -e 's/ //g' > final_$1.csv
...
numprobes_ge=( $(cat final_$1.csv | cut -d "," -f 4 | uniq | tail -10))
